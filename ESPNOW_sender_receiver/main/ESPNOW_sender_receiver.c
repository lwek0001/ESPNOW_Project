...
static ESDA_Payload_Struct outData;     // data to send
static ESDA_controller_struct inData;   // data received by controller
...
int espnow_data_parse(uint8_t *data, uint16_t data_len, uint8_t* state, uint16_t* seq) {
    ...
    *seq = buf->seq_num;
    ...
    if (crc_cal == crc) {
        ...
        inData = buf->payload.controlData;
        // ESP_LOGI(TAG, "crc pass");
        ESP_LOGE(TAG, "Received data:");
        ESP_LOGE(TAG, "xValuePack: %d", inData.xValuePack);
        ESP_LOGE(TAG, "yValuePack: %d", inData.yValuePack);
        ESP_LOGE(TAG, "buttonState: %d", inData.buttonState);
        ESP_LOGE(TAG, "togSwitchVal: %d", inData.togSwitchVal);
...
static esp_err_t my_espnow_init(void) {
    ...
    // create ESPNOW task
    if (xTaskCreate(my_espnow_task, "espnow task", 4096, send_param, 4, NULL) != pdPASS) {
...
void app_main(void)
{
    ...
    // define dummy data
    outData.messageID = SetTargetVelLeft;
    outData.value = 1.23f;
